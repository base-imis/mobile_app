    ...
    assessmentSurveyFields()
      .then((res) => {
        ...
        const initialValues = () => {
          ...
          const validations = {};
          res.data.length > 0 &&
            res.data.map((field, index) => {
              validations[field.name] = Yup.number("Please fill.")
                .required(`${field.label} is required`)
                .max(10000, "Exceeds the maximum distance range")
                .typeError("The field only accepts numerical values.");
              if (field.prefilled == true) {
                v[field.name] = item[field.name] ? item[field.name] : "";
              } else {
                v[field.name] = "123";
              }
            });
          formSchema = Yup.object().shape(validations);
          return v;
        };
        ...
        res.data.map((field) => {
          Yup.object().shape({
            [field.name]: Yup.number("Please fill.")
                      ...
                      {false && (
                        <>
                          ...
                          <Checkbox.Item
                            ...
                            onPress={() =>
                              setFieldValue("vacutugacc", !values.vacutugacc)
                            }
